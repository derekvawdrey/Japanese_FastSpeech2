...
## Setting up audio for training
1. First put the data in raw_data/kokoro/kokoro(or your speaker if you have multiple), you can have multiple speakers but you will need to modify prepare_align.py file
2. Put the aligned TextGrid in /preprocessed_data/kokoro/TextGrid/kokoro(or the speaker)
3. Run python3 preprocess.py config/kokoro/preprocess.yaml
4. You are now ready for training :).

## Requirements.txt
I removed the following from requirements.txt to get it to run on google colab.
```
torch==2.5.1+cu121
